...
async def _async_register_cards_resource(hass: HomeAssistant) -> None:
    ...
    try:
        ...
        if lovelace_data is not None:
            # Modern HA uses a dataclass; older versions use a dict
            ...
            if resources is not None:
                for resource in resources.async_items():
                    ...
                    if "/astronomy-cards/astronomy-cards.js" in url:
        ...
        _LOGGER.warning("API resource registration failed, falling back to storage: %s", err)
...
def _register_resource_via_storage(hass: HomeAssistant) -> None:
    ...
    for item in items:
        if "/astronomy-cards/astronomy-cards.js" in item.get("url", ""):
    ...
    try:
        ...
        if lovelace_data is not None:
            ...
            if resources is not None:
                for resource in resources.async_items():
                    url = resource.get("url", "")
                    if "/astronomy-cards/deepsky-cards.js" in url:
        ...
        _LOGGER.warning("API deepsky-cards resource registration failed, falling back to storage: %s", err)
...
def _register_deepsky_resource_via_storage(hass: HomeAssistant) -> None:
    ...
    for item in items:
        if "/astronomy-cards/deepsky-cards.js" in item.get("url", ""):
